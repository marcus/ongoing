...
| ------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| Exact app-scoped Bun from `.bun-version` alone and frozen dependency graph                       | `deploy/aerie/provision-runtime.sh`, `tests/foundation.test.ts`, `deploy/aerie/release.test.ts`; `bun install --frozen-lockfile`      |
...
| Raw fixed/chunked mutation ceiling before adapter actions and health without catalog leakage     | `src/lib/host/production-server.ts`, `tests/production-server.test.ts`, `deploy/aerie/production-smoke.ts`, `bun run test:production` |
...
| Daily 03:00 scan, deterministic tool PATH, shared storage, distinct labels, and no load loop     | Scan plist, release preflight, stripped-environment `deploy/aerie/release.test.ts`, `plutil -lint`                                    |
...
| Exact-target, two-agent deploy/rollback dry-runs with no remote mutation                         | `deploy/aerie/release.test.ts`, `bun run deploy -- --dry-run ...`, and `bun run rollback -- --dry-run ...`                            |
