...
| ------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| Exact app-scoped Bun from `.bun-version` alone and frozen dependency graph                       | `deploy/aerie/provision-runtime.sh`, `tests/foundation.test.ts`, `tests/release.test.ts`; `bun install --frozen-lockfile`             |
...
| Daily 03:00 scan, deterministic tool PATH, shared storage, distinct labels, and no load loop     | Scan plist, release preflight, stripped-environment `tests/release.test.ts`, `plutil -lint`                                           |
